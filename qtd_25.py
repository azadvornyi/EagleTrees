...
bin_range = np.linspace(8, 12, bin_num+1)
...
small_hosts_set = a =set((small_hosts[0]))

large_hosts = np.where(M_h>1e14)
large_hosts_set =set((large_hosts[0]))


small_split = c.intersection(small_hosts_set)
large_split = c.intersection(large_hosts_set)

small_split = np.array(list(small_split))
print(len(small_split),"small split")

large_split = np.array(list(large_split))
print(len(large_split))
c = np.array(list(c))
...
bin_means_s, bin_edges_s, binnumber_s = stats.binned_statistic( np.log10(m_s[small_split]),quench_timescale_s, statistic='median', bins=bin_range)
#bin_means_l, bin_edges_l, binnumber_l = stats.binned_statistic( np.log10(m_s[large_split]),quench_timescale_l, statistic='median', bins=bin_range)
...
bin_means75_s, bin_edges75_s, binnumber75_s = stats.binned_statistic( np.log10(m_s[small_split]),quench_timescale_s, statistic=lambda x75_s : np.percentile(x75_s,75), bins=bin_range)
bin_means25_s, bin_edges25_s, binnumber25_s = stats.binned_statistic( np.log10(m_s[small_split]),quench_timescale_s, statistic=lambda x25_s : np.percentile(x25_s,25), bins=bin_range)

# bin_means75_l, bin_edges75_l, binnumber75_l = stats.binned_statistic( np.log10(m_s[large_split]),quench_timescale_l, statistic=lambda x75_l : np.percentile(x75_l,75), bins=bin_range)
# bin_means25_l, bin_edges25_l, binnumber25_l = stats.binned_statistic( np.log10(m_s[large_split]),quench_timescale_l, statistic=lambda x25_l : np.percentile(x25_l,25), bins=bin_range)
...
ax5.hist(np.log10(m_s[small_split]), bins = bin_edges_s,align='mid',color='#07575b', label=r'$\mathtt{10^{12}<M_{host}<10^{14}}$' ,edgecolor='#003b46', linewidth=1.2)
#ax5.hist(np.log10(m_s[large_split]), bins=10,label=r'$\mathtt{10^{14}<M_{host}<10^{15}}$', color = '#66a5ad' ,edgecolor='#003b46', linewidth=1.2)
...
ax.errorbar(1,1,1,1, label=r'$\mathtt{10^{12}<M_{host}<10^{14}}$', c='#07575b', lw=1)
...
plt.savefig('qtd_025.pdf', format='pdf', dpi=1200, pad_inches = 0.1, bbox_inches = 'tight')
plt.show()