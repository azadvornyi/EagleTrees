...
M_h, m_s, t_i, t_q,tau_q, sfr,sub = np.loadtxt("the_ultimate_data_RefL0100N1504", unpack=True, usecols=(2, 3, 4, 5, 6,7, 8))
...
c = b.intersection(a)
# c = np.array(list(c))
small_hosts = np.where(sub == 0)
small_hosts_set = set((small_hosts[0]))

large_hosts = np.where(sub != 0)
...
ax5.hist(np.log10(m_s[small_split]), bins=bin_edges_s, align='mid', color='#07575b',
         label=r'$\rm sub = 0$', edgecolor='#003b46', linewidth=1.2, alpha=alpha)
ax5.hist(np.log10(m_s[large_split]), bins=bin_edges_s, align='mid', label=r'$\rm sub >  0$',
         color='#66a5ad', edgecolor='#003b46', linewidth=1.2, alpha=alpha)
...
ax.errorbar(1, 1, 1, 1, label=r'$\rm sub = 0$', c='#07575b', lw=1)

ax.errorbar(1, 1, 1, 1, label=r'$\rm sub > 0$', c='#66a5ad', lw=1)
ax.set_title(r"$\rm QTD \ split \ by \ subgroup \ at \ 3.3R_{vir}$")
ax.set_ylabel(r"$\rm \tau_q \ [Gyr]$")
# plt.plot((binnumber - 0.5) * bin_width, x_pdf, 'g.', alpha=0.5)
#ax5.set_xlabel(r"$\rm log_{10}(M_{\bigstar}/M_{\odot}) $")
...
plt.savefig('qtd_split_by_subgroup_33.pdf', format='pdf', dpi=1200, pad_inches=0.1, bbox_inches='tight')
plt.show()
