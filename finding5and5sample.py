...
host_gid,sat_num,M_h, m_s, t_i, t_q, sfr, t_s = np.loadtxt("pleasehelp", unpack=True, usecols=(0,1,2, 3, 4, 5, 6,7))


# for i in range(len(host_gid)):
#     t_s = (t_q[i]-t_i[i])
#     f = open("pleasehelp", "a")
#     f.write("{0} {1} {2} {3} {4} {5} {6} {7}\n".format(int(host_gid[i]), int(sat_num[i]),M_h[i], m_s[i], t_i[i],
#                                                                              t_q[i], sfr[i], t_s))
#     f.close()

sats = set((np.where((m_s >10**9) & (m_s <10**10))[0]))

hosts_to_keep = set((np.where((M_h >2*10**13) & (M_h <6*10**13))[0]))
...
d = c.intersection(sats)
finalsats = d.intersection(hosts_to_keep)
infall_z2 = set((np.where((t_i>3) & (t_i<4))[0]))
infall_z1 = set((np.where((t_i>5.5) & (t_i<6.5))[0]))

final_sat_z2 = infall_z2.intersection(finalsats)
final_sat_z1 = infall_z1.intersection(finalsats)
final_sat_z2 = np.array(list(final_sat_z2))
final_sat_z1 = np.array(list(final_sat_z1))
print(np.sort(t_s[final_sat_z1]))
print(np.sort(t_s[final_sat_z2]))
#print(times_Gyr([1]))
#print(np.where())
#print(finalsats)
#
# print(np.sort(x))
# print(np.sort(finalsats))
# print(np.sort(host_gid[finalsats]))
#print(finalsats)
#print()
...
lowts_z1 = np.where(t_s[final_sat_z1]<3.7)[0]
hights_z1 = np.where(t_s[final_sat_z1]>7.784)[0]
# print(t_s[final_sat_z1][lowts_z1])
# print(host_gid[final_sat_z1][lowts_z1])
# print(sat_num[final_sat_z1][lowts_z1])
low_host_z1 = host_gid[final_sat_z1][lowts_z1]
low_sat_z1 = sat_num[final_sat_z1][lowts_z1]
high_host_z1 = host_gid[final_sat_z1][hights_z1]
high_sat_z1 = sat_num[final_sat_z1][hights_z1]


lowts_z2 = np.where(t_s[final_sat_z2]<3.4)[0]
hights_z2 = np.where(t_s[final_sat_z2]>8)[0]
print(t_s[final_sat_z2][lowts_z2])
print(host_gid[final_sat_z2][lowts_z2])
print(sat_num[final_sat_z2][lowts_z2])
low_host_z2 = host_gid[final_sat_z2][lowts_z2]
low_sat_z2 = sat_num[final_sat_z2][lowts_z2]
high_host_z2 = host_gid[final_sat_z2][hights_z2]
high_sat_z2 = sat_num[final_sat_z2][hights_z2]

print(len(high_host_z2))
print(len(low_host_z2))

hg_l,fof,sub, dx, dy,dz = np.loadtxt("hostid_satnumber_rvir_33_100.txt", unpack=True, usecols=(0,1,2, 3, 4, 5,))





for i in range(len(low_host_z2)):
    fetch_host_id = np.where(hg_l == int(low_host_z2[i]))[0]

    f = open("mostwanted_z2.txt".format(sim), "a")
    f.write("{0} {1} {2} {3} {4} {5} {6} \n".format(int(hg_l[fetch_host_id]), int(fof[fetch_host_id]),
                                                    int(sub[fetch_host_id]),int(dx[fetch_host_id]),
                                                    int(dy[fetch_host_id]),int(dz[fetch_host_id]),
                                                    int(low_sat_z2[i])))
    f.close()

for i in range(len(high_host_z2)):
    fetch_host_id = np.where(hg_l == int(high_host_z2[i]))[0]
