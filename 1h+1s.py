...
sat_index = int(argv[1])  # int(argv[2])   # select satellite here
...
needed_host = np.where(host_on_edge['gid']==19634929)
dx = int(host_on_edge['dx'][needed_host])
dy = int(host_on_edge['dx'][needed_host])
dz = int(host_on_edge['dx'][needed_host])
print(host_on_edge['gid'][needed_host])
...
normal_sat_query = 'SELECT \
             ...
             and S.Snapnum = 28 \
             and S.MassType_Star between 1E9 and 1E12'.format(sim, 19634929 ,dx,dy,dz, box_size)
...
print(fof_sub_info, 'fof subinfo of a stellite')
...
tree_host_query = 'SELECT \
         ...
         ORDER BY \
             ...
             PROG.Redshift asc, \
             PROG.MassType_Star desc'.format(sim, int(host_on_edge['fof'][needed_host]), int(host_on_edge['sub'][needed_host]))

try:
    with open('simtest/host_1'.format(host_index), 'rb') as f3:
...
print(np.sqrt((tree_host['copx'][z_q] - tree_sat['copx'][z_q])**2 + (tree_host['copy'][z_q] - tree_sat['copy'][z_q])**2 + (tree_host['copz'][z_q] - tree_sat['copz'][z_q])**2),'dist at 1.4 z')
...
try:
    with open("simtest/host_r_vir_1".format(host_index), 'rb') as f4:
...
dirName = 'simtest'.format(host_index)

# Create target directory & all intermediate directories if don't exists
try:
    os.makedirs(dirName)
    with open("simtest/host_1".format(host_index, sim), 'wb+') as f0:
        pickle.dump(tree_host, f0, pickle.HIGHEST_PROTOCOL)
    with open("simtest/host_r_vir_1".format(host_index, sim), 'wb+') as f1:
        pickle.dump(host_r_vir, f1, pickle.HIGHEST_PROTOCOL)
except FileExistsError:
    pass

with open("simtest/sat_{0}".format( sat_index), 'wb+') as f2:
...
plt.semilogy(times_Gyr(tree_host['z']), tree_host['ssfr'] , '-r')
plt.semilogy(times_Gyr(data['z']), data['ssfr'] , '-b', label = "{0}, {1}, {2}".format(fof_sub_info['gid'],fof_sub_info['fof'],fof_sub_info['sub']))
plt.ylim(1e-11,1e-7)
plt.vlines(t_quench, 0, 1)
plt.legend()
plt.savefig('ssfr{0}.pdf'.format(sat_index), format='pdf')
