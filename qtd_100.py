...
ax5.hist(np.log10(m_s[small_split]), bins=bin_edges_s, align='mid', color='#07575b',
         label=r'$\mathtt{10^{13}<M_{host}<10^{14}}$', edgecolor='#003b46', linewidth=1.2)
ax5.hist(np.log10(m_s[large_split]), bins=bin_edges_s, align='mid', label=r'$\mathtt{10^{14}<M_{host}}$',
...
ax.errorbar(1, 1, 1, 1, label=r'$\mathtt{10^{13}<M_{host}<10^{14}}$', c='#07575b', lw=1)

ax.errorbar(1, 1, 1, 1, label=r'$\mathtt{10^{14}<M_{host}}$', c='#66a5ad', lw=1)
...
plt.savefig('qtd_100.pdf', format='pdf', dpi=1200, pad_inches=0.1, bbox_inches='tight')
plt.show()
