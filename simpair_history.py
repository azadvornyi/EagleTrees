...
sat_info_query_1 = 'SELECT \
             PROG.Redshift as z,\
             PROG.MassType_Gas as m_g,\
             PROG.BlackHoleMassAccretionRate as bhmar,\
             PROG.HalfMassRad_DM as half_dm,\
             PROG.ThermalEnergy / (PROG.MassType_Gas+0.0001) as ste \
         FROM \
             {0}_Subhalo as PROG, \
             {0}_Subhalo as DES, \
             {0}_Aperture as AP,\
...
property_sat_1 = sql.execute_query(con, sat_info_query_1)


sat_info_query_2 = 'SELECT \
             PROG.Redshift as z,\
             PROG.HalfMassRad_Gas as half_gas,\
             PROG.HalfMassRad_Star as half_star,\
             PROG.StellarVelDisp as svd\
         FROM \
             {0}_Subhalo as PROG, \
             {0}_Subhalo as DES, \
             {0}_Aperture as AP,\
...
property_sat_2 = sql.execute_query(con, sat_info_query_2)
...
# if not (tree_host['copx'][0] - tree_sat['copx'][0] == 0) and not ((t_quench or t_infall) == 0):
    ...
    f = open("plot_info_for_short_and_long_sats_z2.txt".format(sim), "a")
...
dirName = 'inspection_z2/{0}'.format(sat_flag)
...
#     pass
    ...
    try:
        ...
        with open("inspection_z2/{0}/host_{0}".format(sat_flag), 'wb+') as f0:
            pickle.dump(tree_host, f0, pickle.HIGHEST_PROTOCOL)
        with open("inspection_z2/{0}/host_r_vir_{0}".format(sat_flag), 'wb+') as f1:
            pickle.dump(host_r_vir, f1, pickle.HIGHEST_PROTOCOL)
    except FileExistsError:
        pass

    with open("inspection_z2/{0}/sat_{0}".format(sat_flag), 'wb+') as f2:
        pickle.dump(tree_sat, f2, pickle.HIGHEST_PROTOCOL)
    with open("inspection_z2/{0}/property1_sat_{0}".format(sat_flag), 'wb+') as f3:
        pickle.dump(property_sat_1, f3, pickle.HIGHEST_PROTOCOL)
    with open("inspection_z2/{0}/property2_sat_{0}".format(sat_flag), 'wb+') as f3:
        pickle.dump(property_sat_2, f3, pickle.HIGHEST_PROTOCOL)
